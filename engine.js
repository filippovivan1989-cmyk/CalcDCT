...
// Progressive absolute bands with inclusion offset
...
function applyWidgetRules(PRICE, tariffName, widgets){
  ...
  const payableCount = Math.max(0, capped - free);
  return { max, free, capped, payableCount };
}
...
export function calcTotal(PRICE, inputs){
  ...
  const { payableCount, capped } = applyWidgetRules(PRICE, inputs.tariff, widgets);
  ...
  const pSIP = srvSIP ? (pickPriceByTariff(srvSIP, inputs.tariff) || 0) : 0;
  const pPAY = srvPAY ? (pickPriceByTariff(srvPAY, inputs.tariff) || 0) : 0;
  const I14 = (/папай/i.test(inputs.tariff)) ? 0 : payableCount * pSIP;
  const I15 = payableCount * pPAY;
  ...
  const addonsTotal = addons + I14 + I15 + emailCost;
  const monthlyFlat = t.monthly_flat || 0;
  const total = monthlyFlat + traffic.totalTrafficRub + addonsTotal;

  return { monthlyFlat, traffic, addonsTotal, total, widgets_capped: capped, widgets_payable: payableCount, emailDetails };
}
